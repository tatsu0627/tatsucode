...
export default async function HomePage() {
  ...
  const [studentResult, sessionResult, materialResult] = await Promise.all([
    ...
    db.from("sessions").select("*").order("session_date", { ascending: false }).limit(5),
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-2"><Link className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 font-semibold" href="/sessions/new">{"\u30bb\u30c3\u30b7\u30e7\u30f3\u3092\u8a18\u9332"}</Link><Link className="rounded-xl bg-brand px-4 py-2.5 font-semibold text-white" href="/materials/new">{"\u6559\u6750\u3092\u4f5c\u308b"}</Link></div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section><h2 className="mb-3 text-xl font-semibold">{"\u76f4\u8fd1\u306e\u30bb\u30c3\u30b7\u30e7\u30f3"}</h2>{sessions.length ? <div className="space-y-3">{sessions.map((session) => <Link href={`/sessions/${session.id}/edit`} key={session.id}><Card className="mb-3 transition hover:border-brand"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{session.topic}</p><p className="mt-1 text-sm text-slate-500">{aliases.get(session.student_id) ?? "Alias"} / {formatDate(session.session_date)}</p></div><span className="rounded-lg bg-brand-soft px-2.5 py-1 text-sm font-semibold text-brand">{session.understanding}/5</span></div></Card></Link>)}</div> : <EmptyState title="\u30bb\u30c3\u30b7\u30e7\u30f3\u8a18\u9332\u304c\u3042\u308a\u307e\u305b\u3093" description="\u6388\u696d\u5f8c\u306b\u7406\u89e3\u5ea6\u3068\u6b21\u56de\u76ee\u6a19\u3092\u6b8b\u3057\u307e\u3057\u3087\u3046\u3002" href="/sessions/new" action="\u30bb\u30c3\u30b7\u30e7\u30f3\u3092\u8a18\u9332" />}</section>
      </div>
