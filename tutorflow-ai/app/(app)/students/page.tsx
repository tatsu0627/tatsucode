...
export default async function StudentsPage() {
  ...
  const [studentResult, consultationResult] = await Promise.all([
    db.from("students").select("*").eq("active", true).order("alias"),
    db.from("consultations").select("student_id,consultation_date").order("consultation_date", { ascending: false }),
  ]);
  const students = studentResult.data ?? [];
  const lastDates = new Map<string, string>();
  for (const consultation of consultationResult.data ?? []) if (!lastDates.has(consultation.student_id)) lastDates.set(consultation.student_id, consultation.consultation_date);
