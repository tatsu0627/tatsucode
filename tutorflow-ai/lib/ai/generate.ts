...
import { PROBE_SYSTEM_PROMPT, buildProbePrompt } from "./prompt";
import { mockProbeDraft } from "./mock";
import { probeDraftSchema, type ProbeDraft } from "@/lib/validation/probe-output";
...
export type GenerateProbeRequest = { conceptCode: string; conceptLabel: string };
export type GenerateProbeResult = { draft: ProbeDraft; latencyMs: number; modelLabel: string };

export async function generateProbeDraft(request: GenerateProbeRequest): Promise<GenerateProbeResult> {
  if (process.env.MOCK_AI === "1" && process.env.NODE_ENV !== "production") return mockProbeDraft(request);
  ...
  try {
    response = await getOpenAIClient().responses.parse({
      ...
      reasoning: { effort: reasoningEffort() },
      store: false,
      instructions: PROBE_SYSTEM_PROMPT,
      input: buildProbePrompt(request.conceptCode, request.conceptLabel),
      text: { format: zodTextFormat(probeDraftSchema, "probe_draft") },
    });
  } catch (error) {
    if (error instanceof OpenAI.RateLimitError) throw new AiUnavailableError("rate_limited");
    if (error instanceof OpenAI.APIConnectionError) throw new AiUnavailableError("network");
  ...
  if (response.status !== "completed") throw new AiUnavailableError(`response_${response.status}`);
  if (!response.output_parsed) throw new AiInvalidOutputError("schema_parse_failed");
  if (response.output_parsed.concept_code !== request.conceptCode) throw new AiInvalidOutputError("concept_code_mismatch");
  return { draft: response.output_parsed, latencyMs, modelLabel: model };
}
